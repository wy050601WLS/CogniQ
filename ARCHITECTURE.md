...
**知识问答系统（Knowledge QA）** 是一个基于 RAG（检索增强生成）技术的智能知识问答平台。用户可以上传文件，系统会自动解析、分块、向量化并存储，然后通过 AI 模型对用户的问题进行检索增强生成回答。

### 核心价值
- **知识管理**：支持多种格式文档的上传、解析和管理
- **智能问答**：基于 RAG 技术，AI 可以根据文件内容精准回答问题
...
**是什么？**
RAG 是一种结合检索（Retrieval）和生成（Generation）的 AI 技术。系统先从文件中检索相关文档，然后将这些文档作为上下文提供给大语言模型，生成更准确的回答。
...
async def chat_stream(data, db, user_id):
    ...
    async for chunk, sources in rag_chat(file_id, question, history):
...
| documents | 文件表 | id, owner_id, filename, status, file_path |
| conversations | 对话表 | id, user_id, owner_id, title |
| messages | 消息表 | id, conversation_id, role, content, sources |
| feedbacks | 反馈表 | id, user_id, message_id, rating, comment |
| favorites | 收藏表 | id, user_id, owner_id |
...
users 1──N favorites N──1 documents
```
